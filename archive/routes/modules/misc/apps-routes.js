...
export const loadRoutes = async (app, routes) => {
    await Promise.all(routes.map(async (routersName) => {
        let pathRoute = `/${routersName}`;
        if (routersName === 'latest') {
            pathRoute = '/';
        }
        try {
            await loadRouter(app, DEF_PATH_ROUTES + `/${routersName}/` + DEF_ROUTE_FILE, pathRoute);
        }
        catch (_) {
